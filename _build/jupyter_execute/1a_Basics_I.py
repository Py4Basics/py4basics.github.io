...
# * 浮動小数点型（`float`と表される）
# * 文字列型（`str`と表される; stringの略）
# * ブール型（`bool`と表される; booleanの略）
# 
...
# 整数型は`int`（integer），浮動小数点型は`float`と表示されている。
...
# |     関数 | 例                      | 動作・戻り値 |
# | ----------- | ----------------------- |---|
# | `type()` | `type(10)`　→　`int`    | データ型を返す |
# | `int()` | `int(10.0)`　→　`10`       | 整数型に変換 |
# | `float()` | `float('10.0')`　→　`10.0` | 不動小数点型に変換 |
# | `str()` | `str(10)`　→　`'10'` | 文字列型に変換 |
# | `bool()` | `bool(0)`　→　`False` | 真偽値を返す |
...
# `5➗２=2余り1`となるが，`5%2`は`1`を返し，`5//2`は`2`を返すことになる。
# 
# ````{note}
# 剰余演算と切り捨て演算には次の関係が成立する（即ち，`True`を返す）。
# 
# ```
# a　== (a//b)(b) + (a%b)
# ```
# `a,b>0`の場合は問題なく理解できる。しかし，`a`もしくは`b`もしくは両方が負の値を取るとエキゾチックな値になるので注意する必要がある。基本的に`//`は「床関数」と同義と考え，上の関係が満たされるように`%`の値が決まることになる。このサイトでは`a,b>0`のみを考える。
# ````
...
# 
# ```{note}
# `x = 10`を考えてみよう。Pythonの[公式ドキュメント](https://docs.python.org/3/reference/simple_stmts.html)では，このように`=`が使われるコードを assignment statement と読んでおり，[日本版](https://docs.python.org/ja/3/reference/simple_stmts.html)は「代入文」と訳している。これに従うと`=`は代入演算子と呼ぶことができるだろう。しかし英語では assignment（割り当て）と substitution（代入）は若干異なる意味となり，その違いが参照記号と実態の関係を反映していると考えられる。このサイトでは，参照記号と実態の違いを強調するために「割り当てる（assign）」という表現を使っている。
# ```
...
# またリストの中にリストや他のデータ型（以下で説明するタプルや辞書など）を入れることも可能だ。例えば，
...
# タプルの中にタプルや他のデータ型（リストや以下で説明する辞書など）を入れることも可能だ。例えば，
...
# また，上で定義した`list1`をタプルに変換したい場合は`tuple()`を使う。
...
your_tuple = (100, (200, 300), [400, 500, 600])


# この場合でも考え方は変わらない。例えば，`['B','C']`を抽出したいとしよう。`1`番目の要素なので次のコードとなる。
...
# 更に，`'B'`を抽出したいとしよう。その場合は，次のように抽出する要素番号を順を追って指定すれば良い。
...
# タプルの場合も同じである。例えば，`500`を抽出するとしよう。
