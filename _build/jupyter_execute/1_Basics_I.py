...
# 近似値を見る限り，`0.3 == 0.1 + 0.1 + 0.1`が`False`を返すことは正しい結果である。この事から，次のことが言える。
# > 浮動小数点型に`==`を使い真偽を確かめるのは**危険**である。
# 
# ````{note}
# 解決策として`numpy`の`isclose()`関数を使うことができる。この関数は２つの値が十分に近い場合は`True`を返す。では、どれだけ近い場合に`True`を返すのか？詳細は[マニュアル](https://numpy.org/doc/stable/reference/generated/numpy.isclose.html)を参照。次のコードは`True`を返すことになる。
# ```
# import numpy as np
