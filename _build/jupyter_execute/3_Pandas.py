...
# `df`のメソッド`.mean()`を使うと列の平均を簡単に計算できる。
...
# 
# またステップ２の計算対象となる列を複数指定しても構わない。ただその場合は，リストとして列を指定すること。
...
# 結果は`DataFrame`として返されている。
# 
# 同様にステップ１のグループ分けする列も複数選択可能である。その際もリストとして列を指定する必要がある。`df`のような小さな`DataFrame`では`.groupby`の威力はあまりピンとこないかも知れない。しかし大きな`DataFrame`を使うとその恩恵を強く感じることだろう。[Gapminder](https://py4basics.github.io/Gapminder.html)ではマクロ経済データを使い`.groupby()`の使い方の例を示している。興味がある人は是非！
