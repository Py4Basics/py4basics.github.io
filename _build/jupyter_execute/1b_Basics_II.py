...
# この例を使って`for`ループの２つの書き方を説明する。目的は動学的な変数の変化を考えるためのコード作成の基礎作りである。次のコードが一つ目の方法である。ここでは２つ目の書き方と比較するために，初期値`0`を最初から`my_list`に置いている。（もちろん，そうせずにコードを書くことができるので考えてみよう。）
