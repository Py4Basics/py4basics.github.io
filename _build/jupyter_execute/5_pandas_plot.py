...
# タイトルのフォント・サイズの指定，横軸と縦軸のラベルとフォント・サイズの指定をおこなう場合は、`plot()`の引数ではなく下で説明する方法でおこなう。この方法を理解するために、`Python`は次の手順で描画していることをイメージして欲しい。ここで重要なのは「図」，「軸」と「プロット」の違いである。次の図のような構成となっている。
# 
...
#     * 横軸・縦軸とは異なる。
#     * １つのグラフ表示する区域。
#     * 「図」上に複数のグラフを表示する場合は複数区域に分けることになる。
#     * ここにグラフのタイトルや縦軸・横軸のラベルなどを追加することになる。
# 1. プロット（plot）
#     * １つのグラフの主な部分。
#     * １つの「軸」内に表示されるグラフ。
#     * 「図」上に複数のプロットを表示するためには複数の「軸」を用意する必要がる。

# 従って，概ね次のように理解して良いだろう。
# * 上で説明した「基本的な引数」はプロット（plot）の引数（`title`もあるがオマケのようなものだろう）
# * タイトルや縦・横軸ラベルは「軸」の引数
# 
# では実際に手順を説明する。
