...
# 上で`x = x + 1`が出てきたが、これを短縮して`x += x`と書くことができる。次のコードを考えようてみよう。
...
# 要素を削除するには色々な方法が用意されているが，最も簡単なのは`del`（deleteの略）を使うことだろう。`del`の後に削除したい要素を書き実行する竹である。もちろんスライシングにも対応している。例えば，
...
# 辞書はキーと値のペアとなっているが，変更できるのは値だけである。方法は，`=`の左辺に変更したい値を選び，右辺に変更後の値を配置するだけである。例えば，`dict0`の`失業`を`完全雇用`に入れ替えてみよう。
