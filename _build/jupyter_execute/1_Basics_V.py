...
# * 第２引数：最大値（整数型）
# 
# 次のコードは$[1,10]$の間の整数を返す（`1`と`10`は戻り値に含まれる）。
# ```
# random.randint(1, 10)
# ```
# 
# 次のコードは，サイコロの目（1~6）の１つを返す。
...
# ````{note}
# 内包表記に`_`を使っている。`i`や`j`を使っても良いが，回数を数える以外に役目はない。その様な場合に良く使われるのが`_`である。`for`ループでも同じことが言える。次のコードが例として挙げられる。
# ```
# for _ in range(5):
#     print('hello')
# ```
...
# 
# 例えば，カレーを作るとしよう。アルゴリズムはその場合のレシピである。ある程度料理ができる人であれば，書かれている手順に沿って作れば美味しいカレーが出来上がるレシピがアルゴリズムである。様々な問題には異なるアルゴリズムがある。また１つの問題には複数のアルゴリズムが存在する。カレーでも色々なレシピがあるのとおなじである。
# 
