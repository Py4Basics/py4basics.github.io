...
# ＜`#1`の`if money:`について＞
# * `if money != 0`と同じである。だが`if money:`の方がより簡単で`Python`的（Pythonic）だと言える。
# * `if money:`は「もしお金があれば」と読めるため，より直感的でもある。
...
# ＜コードの説明＞
# * `#1`：`k_list`の最初の要素として`0`を入れている。以下では`k_list`に値を追加して目的のリストを作成する。
# * `#2`：ループの開始。
...
# * １回目のループ
#     * `#3`：`k_list`の`0`番目の要素である`0`を抽出し，それに`10`をを足すことにより右辺は`10`となる。それを`k`に割り当てる。
#     * `#4`：`k`の値`10`を`k_list`に追加する。`10`は`k_list`の1番目の要素となる。
# * ２回目のループ
#     * `#3`：`k_list`の`1`番目の要素である`10`を抽出し，それに`10`をを足すことにより右辺は`20`となる。それを`k`に割り当てる。
#     * `#4`：`k`の値`20`を`k_list`に追加する。`20`は`k_list`の2番目の要素となる。
# * ３回目のループ
#     * `#3`：`k_list`の`2`番目の要素である`20`を抽出し，それに`10`をを足すことにより右辺は`30`となる。それを`k`に割り当てる。
...
# * `#2`：最初の`k`の値`0`からなるリストの作成する。以下では`k_list`に値を追加して目的のリストを作成する。
# * `#3`：ループの開始。
#     * イタラブルとして`range(9)`を使っている。`list(range(9))`と置いても良いが，`range(9)`で役目は果たす。
#     * `range(9)`は`0`から`8`までの整数を準備し，9回ループが実行されることになる。
#     * `0`から`8`までの整数を順次`t`に割り当てることになる。
...
# * １回目のループ
#     * `#4`：右辺の`k`の値は１行目で割り当てた`0`である。それに`10`を足し右辺は`10`となり，それを左辺の`k`に割り当てる。この時点で，１行目の`k=0`は「無効」となり，`k=10`として更新される。
...
# * １回目のループ
#     * `#1`：`enumerate(values)`から`(0,100.0)`が引き出され，それぞれが`i`と`v`に割り当てられる。
#     * `#2`：`print()`関数を使い`i`と`v`を表示する。
#         * f-stringを使っている。
#         * `{v:.1f}`の`:.1f`は小数点第一位までの表示を指定している。
# * ２回目のループ
#     * `#1`：`enumerate(values)`から`(1,102.0)`が引き出され，それぞれが`i`と`v`に割り当てられる。
#     * `#2`：`print()`関数を使い`i`と`v`を表示する。
# * 同様のループが合計6回おこなわれる。
