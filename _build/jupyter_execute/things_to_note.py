...
# 
# 解決策として`numpy`の`isclose()`関数を使うことができる。この関数は２つの値が十分に近い場合は`True`を返す。では、どれだけ近い場合に`True`を返すのか？詳細は[マニュアル](https://numpy.org/doc/stable/reference/generated/numpy.isclose.html)を参照。
